---
layout: post
title:  "RxJava"
date:   2018-05-18 09:11:11 +0800
categories: Android
tags: RxJava
author: pepe
description: 『 RxJava系列一 』
---

[给 Android 开发者的 RxJava 详解](http://gank.io/post/560e15be2dca930e00da1083)

[rengwuxian/RxJavaSamples: RxJava 2 和 Retrofit 结合使用的几个最常见使用方式举例](https://github.com/rengwuxian/RxJavaSamples)




[RxJava 参考资源 - 云在千峰](http://blog.chengyunfeng.com/?p=985)

[Intro To RxJava 系列教程 总结 - 云在千峰](http://blog.chengyunfeng.com/?p=983)

[RxJava（十五）RxJava线程的自由切换 - Chiclaim - CSDN博客](https://blog.csdn.net/johnny901114/article/details/80032801)

[RxJava开发精要8 - 与REST无缝结合-RxJava和Retrofit - CSDN博客](https://blog.csdn.net/bboyfeiyu/article/details/50512464)

[lzyzsd/Awesome-RxJava: RxJava resources](https://github.com/lzyzsd/Awesome-RxJava)

[RxJava----简介 - CSDN博客](https://blog.csdn.net/qq_20198405/article/details/51243098)

[这可能是最好的RxJava 2.x 教程（完结版） - 简书](https://www.jianshu.com/p/0cd258eecf60)

[nanchen2251/RxJava2Examples: RxJava2 Examples —— 这可能是从 RxJava1 跳到 RxJava2（学习 RxJava2 ）最好的例子 Demo：https://github.com/nanchen2251/RxJava2Examples](https://github.com/nanchen2251/RxJava2Examples)

[androidmalin/RxjavaSample: RxJava Sample](https://github.com/androidmalin/RxjavaSample)

[kaushikgopal/RxJava-Android-Samples: Learning RxJava for Android by example](https://github.com/kaushikgopal/RxJava-Android-Samples)

[amitshekhariitbhu/RxJava2-Android-Samples: RxJava 2 Android Examples - Migration From RxJava 1 to RxJava 2 - How to use RxJava 2 in Android](https://github.com/amitshekhariitbhu/RxJava2-Android-Samples)

[RxJava2 实战知识梳理(9) - 使用 timer/interval/delay 实现任务调度 - 简书](https://www.jianshu.com/p/6db5d213c9c6)

[Android RxJava 实际应用讲解：从磁盘 / 内存缓存中 获取缓存数据 - 简书](https://www.jianshu.com/p/6f3b6b934787)

[RxJava的四种Subjects：PublishSubject、ReplaySubject、BehaviorSubject、AsyncSubject的理解 - yongfengnice - 博客园](https://www.cnblogs.com/yongfengnice/p/10229473.html)



