...
> 这里的自身是指`『View控件』，scrollTo和scrollBy，改变的是『View内容』，对event.getX()和event.getY()无影响，`event.getX()`和`event.getRawX()`相同。`如果用属性动画，改变了『View控件』 的`「实时位置信息和偏移值」`，那么`event.getX()`和`event.getRawX()`就不相同了。
